...
@author: Junyue
beth tweaking this to add well information for ligation, rt, and pcr to header line of each read and also add sample name to each read. 
Eventually might want to also separate into different folders for each sample.
"""
        ...
        if tmp_lig in ligation_barcode_list:
            ...
            if ligation_bc_match in LIGwells_dict:
                ...
                if target_RT in RT_barcode_list:
                    ...
                    if barcode in RTsamplename_dict: #now check if it's even an RT barcode that you used and connect to samplename 
                        ...
                        first_line = '@LIG-' + ligindx + '_RT-' + RTwell + ',PCR-' + pcrwell + ',' + UMI + ',' + samplename + '\n'  #adding samplename 
...
def attach_UMI_files(input_folder, pcrwell, output_folder, ligation_barcode_file, LIGlist, RT_barcode_file, RT_sample, core):
    ...
    func = partial(UMI_attach_read2_barcode_list, input_folder = input_folder, output_folder=output_folder, ligation_barcode_list = ligation_barcode_list, RT_barcode_list=RT_barcode_list, RTsamplename_dict = RTsamplename_dict, RTwells_dict = RTwells_dict, LIGwells_dict= LIGwells_dict, mismatch_rate = 1)
